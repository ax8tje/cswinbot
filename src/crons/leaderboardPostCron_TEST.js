...
async function generateLeaderboard(clan, clanMembers) {
    ...
    return new EmbedBuilder()
        .setTitle(`🏆 ${clan.clanTag || clan.clanName} — Daily Leaderboard [TEST]`)
        .setDescription("```\n" + lines.join("\n") + "\n```")
        .setColor(0xff9900) // Orange color for testing
        .setFooter({ text: `${clan.clanName} • ${leaderboard.length} members • TEST MODE` })
        .setTimestamp();
}

async function postLeaderboards(client) {
    console.log(`[LeaderboardPost TEST] Starting test post...`);
    ...
    for (const [clanId, clan] of Object.entries(clansData.clans)) {
        ...
        if (!clan.leaderboardChannelId) {
            console.log(`[LeaderboardPost TEST] Clan ${clan.clanName} has no channel set, skipping...`);
            continue;
        }
        ...
        try {
            ...
            if (clanMembers.length === 0) {
                console.log(`[LeaderboardPost TEST] Clan ${clan.clanName} has no members, skipping...`);
                continue;
            }
            ...
            if (!channel) {
                console.warn(`[LeaderboardPost TEST] Channel ${clan.leaderboardChannelId} not found for ${clan.clanName}`);
                failCount++;
                continue;
            }
            ...
            console.log(`[LeaderboardPost TEST] Posted leaderboard for ${clan.clanName}`);
            successCount++;
            ...
            console.error(`[LeaderboardPost TEST] Error posting for ${clan.clanName}:`, err.message);
            failCount++;
        }
    }

    console.log(`[LeaderboardPost TEST] Complete: ${successCount} success, ${failCount} failed`);
}

module.exports = (client) => {
    // TEST: Post every 2 minutes
    cron.schedule('*/2 * * * *', async () => {
        try {
            await postLeaderboards(client);
        } catch (err) {
            console.error('[LeaderboardPost TEST] Cron error:', err);
        }
    });

    console.log('⚠️  TEST MODE: Leaderboard post cron scheduled:');
    console.log('   - Test post: Every 2 minutes');
    console.log('   - Replace with production version when done testing!');
};